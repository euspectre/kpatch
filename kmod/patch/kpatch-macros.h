...
#define KPATCH_POST_PATCH_HOOK(_fn) \
	...
	static struct kpatch_post_patch kpatch_post_patch_data __used __section(.kpatch.hooks.post_patch) = { \
		.fn = _fn, \
		.objname = NULL \
	};
#define KPATCH_PRE_UNPATCH_HOOK(_fn) \
	static inline kpatch_pre_unpatchcall_t __pre_unpatchtest(void) { return _fn; } \
	static struct kpatch_pre_unpatch kpatch_pre_unpatch_data __used __section(.kpatch.hooks.pre_unpatch) = { \
		.fn = _fn, \
		.objname = NULL \
	};
#define KPATCH_POST_UNPATCH_HOOK(_fn) \
	static inline kpatch_post_unpatchcall_t __post_unpatchtest(void) { return _fn; } \
	static struct kpatch_post_unpatch kpatch_post_unpatch_data __used __section(.kpatch.hooks.post_unpatch) = { \
		.fn = _fn, \
		.objname = NULL \
	};
