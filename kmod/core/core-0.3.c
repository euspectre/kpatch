...
static int kpatch_find_callback(void *data, const char *name,
				struct module *mod, unsigned long addr)
{
	struct kpatch_find_arg *args = data;
				     ...
				     unsigned long sympos, unsigned long *addr)
{
	...
	/* kpatch_find_callback() expects objname to be NULL for vmlinux. */
	if (!strcmp(objname, "vmlinux"))
		args.objname = NULL;

	mutex_lock(&module_mutex);
	kallsyms_on_each_symbol(kpatch_find_callback, &args);
 ...
 */
static unsigned long kpatch_find_external_symbol(struct module *pmod,
						 const char *name,
						 unsigned long *addr)
{
	 ...
	 */
	return kpatch_find_object_symbol(pmod->name, name, 0, addr);
}
				    ...
				    struct kpatch_object *object)
{
	...
	list_for_each_entry(dynrela, &object->dynrelas, list) {
		...
		if (dynrela->external) {
			...
			ret = kpatch_find_external_symbol(kpmod->mod,
							  dynrela->name,
							  &src);
		} else {
		...
		case R_X86_64_PC32:
			loc = dynrela->dest;
			val = (u32)(src + dynrela->addend -
				    dynrela->dest);
			size = 4;
			break;
		case R_X86_64_32S:
			loc = dynrela->dest;
			val = (s32)src + dynrela->addend;
			size = 4;
			break;
		case R_X86_64_64:
			loc = dynrela->dest;
			      ...
			      struct kpatch_object *object)
{
	...
	list_for_each_entry(func, &object->funcs, list) {
		ret = kpatch_find_object_symbol(
			object->name,
			func->name, func->old_sympos, &func->old_addr);
		if (ret) {
			func_err = func;
			goto err_ftrace;
		}
