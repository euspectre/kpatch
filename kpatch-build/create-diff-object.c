...
static const struct rela *toc_rela(const struct rela *rela)
{
	if (strcmp(rela->sym->sec->name, ".toc"))
		return rela;
...
{
	return ((toc_rela(rela))->sym->type == STT_FUNC &&
		(rela->type == R_X86_64_32S ||
...
{
	int nr, index, i;
	struct section *sec, *ksym_sec, *krela_sec, *toc_sec, *toc_relasec;
	...
	int *toc_dynrela_off, toc_sec_off, skip_toc_off, skip, off_idx;
	Elf_Data *toc_data;
	...
	/* Allocate toc dynrela array to half of upper bound (nr/2) */
	toc_dynrela_off = malloc(sizeof(int) * (nr/2));
	memset(toc_dynrela_off, 0, sizeof(*toc_dynrela_off));
	off_idx = 0;

	/* Find the offset of last rela in .rela.toc */
	rela = list_entry(toc_relasec->relas.prev, struct rela, list);
	toc_sec_off = (rela->offset + 8);

	/* populate sections */
	index = skip_toc_off = skip = 0;
	list_for_each_entry(sec, &kelf->sections, list) {
		if (!is_rela_section(sec))
			continue;
		/*
		 * .rela.toc will be checked for dynrelas after processing
		 * all sections. We might, need to create duplicate .toc
		 * rela entries for the callbacks, workqueues to original
		 * function address. These duplicate symbols, will be populated
		 * as dynrela for .toc, as well as updated in section rela
		 * loading them manually to newly populated offset.
		 */
		if (!strcmp(sec->name, ".rela.kpatch.funcs") ||
...
toc_sec_dynrela:
		list_for_each_entry_safe(rela, safe, &sec->relas, list) {
			if (!strcmp(sec->name, ".rela.toc")) {
				/* skip the dynrela toc entries generated by us */
				if (!skip && skip_toc_off &&
				    rela->offset == skip_toc_off)
					skip = 1;

				for (i = 0; !skip && i < off_idx; i++) {
					if (toc_dynrela_off[i] == rela->offset) {
						ALLOC_LINK(rela2, &toc_relasec->relas);
						memcpy(&rela2->rela, &rela->rela,
								sizeof(rela->rela));
						rela2->sym = rela->sym;
						rela2->type = rela->type;
						rela2->addend = rela->addend;
						rela2->offset = toc_sec_off;
						rela2->string = rela->string;

						if (!skip_toc_off)
							skip_toc_off = rela2->offset;
						toc_sec_off += 8;
						break;
					}
				}
				if (!skip && rela->sym->sec)
					continue;
			} else {
				...
				if (!strcmp(rela->sym->name, ".toc")) {
					/*
					 * Check if .toc+offset is already added
					 * to array toc dynrela.
					 */
					for (i = 0; i < off_idx; i++) {
						if (toc_dynrela_off[i] == rela->addend) {
							rela->addend =
								toc_sec_off - ((off_idx - i) * 8);
							break;
						}
					}
					if (i != off_idx)
					...
					toc_dynrela_off[off_idx++] = rela->addend;
					rela->addend = toc_sec_off;
					toc_sec_off += 8;
					continue;
				}
			}
		...
		/* Don't loop with .rela.toc->list.next, short circuit it */
		if (!strcmp(sec->name, ".rela.toc"))
			break;

		/* Process .rela.toc at the end of section list */
		if (sec->list.next == &kelf->sections) {
			sec = toc_relasec;

			if (!off_idx)
				goto toc_sec_dynrela;

			toc_sec = toc_relasec->base;
			toc_data = malloc(toc_sec->data->d_size + (off_idx * 8));
			if (!toc_data)
				ERROR("malloc");
			memcpy(toc_data, toc_sec->data->d_buf, toc_sec->data->d_size);
			toc_sec->data->d_buf = toc_data;
			toc_sec->data->d_size += (off_idx * 8);
			toc_sec_off-= (off_idx * 8);
			goto toc_sec_dynrela;
		}
	}
